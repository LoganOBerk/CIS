/*
*Author: Luis E. Ortiz
*Purpose: Code for simple stock market trading simulator using priority queues
based on heaps and
...
// prints out a string representation of the whole BST using a reverse inorder
//traversal
void
...
// PRECONDITION: the tree is not empty; and the left or right subtree, or both, of
//w are empty
// POSTCONDITION: the size of the BT is reduced by 1 after w is properly removed
//from the BT
Elem*
...
# (True/False), the output is the first ancestor of w that has a left/right
relationship to its child that is also an ancestor of w
*/
BT::Node*
...
# flag (True/False), the output is the (leftmost/rightmost) node in the subtree
rooted at w
*/
BT::Node*
...
// INPUT: p, a node in the BT or NULL; c, a node in the BT or NULL; and a Boolean
//flag, isLeft, signaling whether c should become the left child of p
// PRECONDITION:
...
//to the root; the last node is removed and the new last node is set; a down - heap
//bubbling operation is performed at the root with the new copied element to maintain
//the heap - order property as needed
void
Heap::removeMin() {
	// NAME: Logan Berk
	// Your code here
...
//after performing an insertion to maintain the heap - order property
void
Heap::upHeapBubbling() {
	// NAME: Logan Berk
	// Your code here
...
//heap is not empty or it has at least two nodes, after performing a removal to
...
// INPUT: an element e and a Boolean flag signaling whether e corresponds to a buy
//transaction
// PRECONDITION: e is non-NULL, as are its key and value
// POSTCONDITION: the transaction is inserted into the ledger/book for the
//corresponding trader, and the trader record is updated(i.e., the trader's
...
// INPUT: an element e
// POSTCONDITION: a buy transaction is inserted into the ledger/book for the
//corresponding trader, and the trader record is updated to increase the trader's
//holdings by the number of shares bought and reduce the trader's balance by the
...
// INPUT: an element e
// POSTCONDITION: a sell transaction is inserted into the ledger/book for the
//corresponding trader, and the trader record is updated to reduce its holdings by
//the number of shares sold and increase the trader's balance by the amount of money
...
// INPUT: the number of shares and price involved in the trade, the trader's ID,
//and the time order was placed
// POSTCONDITION: a new element for the order is added to the respective limit-
//order book for the stock market depending on the trade type
void
...
// INPUT: the number of shares and price for the buy order placed by the trader
//with the given input id, and the time the buy order was placed
// POSTCONDITION: a new element for the buy order is added to the respective buy
//limit - order book for the stock market
void
StockMarket::buyAux(int num, double price, int id, int t) {
	transAux(num, price, id, t, true);
}
// INPUT: the number of shares and price for the sell order placed by the trader
//with the given input id, and the time the sell order was placed
// POSTCONDITION: a new element for the sell order is added to the respective sell
//limit - order book for the stock market
void
StockMarket::sellAux(int num, double price, int id, int t) {
	transAux(num, price, id, t, false);
}
// INPUT: the price and number of shares for the buy order placed by the trader
//with the given input id
// POSTCONDITION: a new element for the buy order is added to the respective buy
//limit - order book for the stock market and a trade is executed if there is a
//matching sell order already in the limit - order books for the stock market
void
StockMarket::buy(double price, int num, int id) {
	buyAux(num, price, id, counter++);
	trade();
}
// INPUT: the price and number of shares for the sell order placed by the trader
//with the given input id
// POSTCONDITION: a new element for the sell order is added to the respective sell
//limit - order book for the stock market and a trade is executed if there is a
//matching buy order already in the limit - order books for the stock market
void
StockMarket::sell(double price, int num, int id) {
	sellAux(num, price, id, counter++);
	trade();
}
// PRECONDITION: there are matching orders in the limit-order books for the stock
//market
// POSTCONDITION: any possible trade is executed, and properly documented/recorded;
//the limit - order books for the stock market are properly updated and maintained; the
//stock market's bank balance is increased if there is a margin over the markets'
//spread(i.e., the buy limit - order price is higher than the sell limit - order price)
void
...
// POSTCONDITION: all possible trades are processed/executed and
//recorded / documented, the market's limit-order books are properly
//updated / maintained, and the market profit from the respective trades(if any) is
//updated / increased
void
StockMarket::trade() {
